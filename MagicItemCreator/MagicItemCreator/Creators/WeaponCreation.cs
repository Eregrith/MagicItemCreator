...
namespace MagicItemCreator.Creators
{
    public static class WeaponCreation
    {
        ...
        public static List<WeaponSpecialAbilitiesTableLine> MeleeWeaponSpecialAbilitiesTable { get; set; }
        public static List<WeaponSpecialAbilitiesTableLine> RangedWeaponSpecialAbilitiesTable { get; set; }
        ...
        public static MagicWeapon Create(ItemQuality quality)
        {
            ...
            WeaponTableLine ligne = WeaponsTable.GetLineFromDice(de, quality);
        ...
        private static MagicWeapon CreateSpecific(ItemQuality quality)
        {
            MagicWeapon item = new MagicWeapon(quality);
        ...
        private static MagicWeapon CreateWithAbilityAndReroll(ItemQuality quality)
        {
            //1. Roll pour avoir l'alteration
            WeaponTableLine secondRoll;
            do
            {
                int alteration = Dices.d100();

                secondRoll = WeaponsTable.GetLineFromDice(alteration, quality);
            } while (secondRoll.Create == null); //On reroll tant qu'on a pas trouvé une ligne qu'on peut utiliser en tant que deuxième roll (si Create existe c'est un cas spécial et donc on ne peut pas l'utiliser)
            ...
            //3. On trouve le prix
            baseWeapon.Price = WeaponsTable.GetLineFromAlterationBonus(baseWeapon.AlterationBonus + baseWeapon.BasePriceModifier).Price;
